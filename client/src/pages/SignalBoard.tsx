  ...
  if (!isAuthenticated) {
    return (
      <main className="signal-board signal-board--locked">
        ...
        <Link href="/" className="platform-return">RETURN TO UNIVERSE <b>↑</b></Link>
      </main>
    );
  }
    ...
    <main className="signal-board">
      ...
      <section className="signal-board__content">
        <div className="board-group">
          <div className="board-group__header"><p>01 / SAVED TRAVEL ROUTES</p><Link href="/travel">FIND A TRIP <b>↗</b></Link></div>
          {destinations.length ? <div className="board-list">{destinations.map(signal => <Link href={signal.href || "/travel"} key={signal.id}><span>ROUTE</span><div><strong>{signal.title}</strong><p>{signal.subtitle || "Travel field"}</p></div><b>↗</b></Link>)}</div> : <p className="board-empty">NO SAVED TRAVEL ROUTES YET. PLAN A TRIP, THEN CHOOSE “SAVE TO MY BOARD”.</p>}
        </div>
        <div className="board-group board-group--music">
          <div className="board-group__header"><p>02 / SAVED MUSIC</p><Link href="/music">PLAY MUSIC <b>↗</b></Link></div>
          {playlists.length ? <div className="board-list">{playlists.map(signal => <Link href={signal.href || "/music"} key={signal.id}><span>PLAY</span><div><strong>{signal.title}</strong><p>{signal.subtitle || "Music field"}</p></div><b>↗</b></Link>)}</div> : <p className="board-empty">NO SAVED MUSIC YET. OPEN A LISTENING FIELD, THEN CHOOSE “SAVE TO MY BOARD”.</p>}
        </div>
      </section>
    </main>
  );
